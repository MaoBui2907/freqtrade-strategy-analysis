...
def get_backtestings(db: Database):
    ...
    return [{
        ...
        "strategy_group_id": str(r.get("strategy_group_id", "")),
    } for r in list(res)]
...
def get_backtesting(db: Database, id: str):
    ...
    return {
        ...
        "strategy_group_id": str(res["strategy_group_id"]),
...
def get_backtesting_to_process(db: Database, id: str):
    ...
    if not res:
        return None
    res = res[0]
    strategies = list(db.get_collection("strategies").find({"name": {"$in": res.get('strategy_group', [{}])[0].get('strategies', [])}}))
...
def create_backtesting(db: Database, backtesting: dict):
    res = db.get_collection("backtestings").insert_one({
        ...
        "strategy_group_id": ObjectId(backtesting.get('strategy_group_id', '')),
